...
impl ParallelCsvProcessor {
    ...
    // Optional: Alternative constructor method can remain similar to previous implementation
    pub fn with_pool(pool: &Arc<Pool<SqliteConnectionManager>>, num_workers: Option<usize>, db_state: &State<DbState>) -> Self {
        let num_workers = num_workers.unwrap_or_else(|| {
            thread::available_parallelism()
                .map(|n| n.get())
                .unwrap_or(4)
    ...
    pub fn process_csv_data(
        ...
        records.par_chunks(chunk_size)
            .for_each_with(
                ...
                |(sender, errors), chunk| {
                    // Create a new connection from the pool for each thread
                    let thread_conn = match pool.get() {
                        Ok(conn) => conn,
                        Err(e) => {
                            errors.lock().unwrap().push(format!("Failed to get connection: {}", e));
                            return;
                        }
                    };
